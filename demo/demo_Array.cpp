...
int main()
{
  ...
  BIC::foreach(BIC::fixed<int, 0>, BIC::fixed<int, 10>, [](const auto i) { std::println("  * i = {} is i fixed ? {}", i, isFixed(i)); });
  std::println("Iterating over a reversed range from Fixed<int,9> to Fixed<int,0>");
  BIC::foreach(BIC::fixed<int, 9>, BIC::fixed<int, 0>, BIC::fixed<int, -1>, [](const auto i) { std::println("  * i = {} is i fixed ? {}", i, isFixed(i)); });
